...
const initialState: GetInTouchFormState = {
  name: '',
  phone: '',
  businessName: '',
  industry: '',
...
const GetInTouch = ({ onClose }: GetInTouchProps) => {
  const [form, setForm] = useState<GetInTouchFormState>(initialState);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await addDoc(collection(db, 'getintouch'), form);
  ...
  return (
    ...
    >
      <h2 className="text-2xl font-bold mb-6 text-center text-[var(--primary-color)]">Get in Touch</h2>
